---
name: dhf-outlook-mail-task
description: 调用 DHF Agent 任务市场中的 Outlook 邮件发送任务 (juHtXM)。使用浏览器自动发送邮件，支持收件人选择、主题、正文和附件上传
version: 1.0.0
metadata:
  tags: [dhf, automation, email, outlook-mail, task]
...
自动调用 DHF Agent **任务**，通过浏览器发送 Outlook 邮件。
...
```bash
# 基本调用
/dhf-outlook-mail-task --to "recipient@example.com" --subject "邮件主题" --body "邮件内容"

# 带附件
/dhf-outlook-mail-task --to "recipient@example.com" --subject "邮件主题" --body "邮件内容" --attachment "/path/to/file.pdf"

# 多个收件人
/dhf-outlook-mail-task --to "a@example.com,b@example.com" --subject "邮件主题" --body "邮件内容"

# 抄送
/dhf-outlook-mail-task --to "recipient@example.com" --cc "cc@example.com" --subject "邮件主题" --body "邮件内容"
```
...
4. 等待 DHF Agent 启动完成（最多 30 秒）
   ↓
5. 调用任务 (task_id: juHtXM)
   ↓
6. 自动打开浏览器
   ↓
7. 登录 Outlook 邮箱（如果未登录）
   ↓
8. 填写邮件信息
   ↓
9. 上传附件（如果有）
