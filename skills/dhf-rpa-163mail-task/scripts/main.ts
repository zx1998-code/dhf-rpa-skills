...
// Lock file to prevent duplicate execution (fixed name, not PID-based)
const LOCK_FILE = join(tmpdir(), "dhf-163mail-task.lock");
...
// Show help
function showHelp() {
  printHeader("DHF 163 Mail Task - 163 邮件自动发送");
  console.log();
  log("用法:", colors.bright);
  console.log();
  log("  /dhf-163mail-task [options]", colors.cyan);
  console.log();
  ...
  log("  /dhf-163mail-task --to \"friend@163.com\" --subject \"聚会\" --body \"周末有空吗？\"", colors.green);
  console.log();
  log("  # 发送带附件的邮件", colors.gray);
  log("  /dhf-163mail-task --to \"colleague@163.com\" --subject \"报告\" \\ ", colors.green);
  log("    --body \"附件是报告\" --attachment \"/path/to/report.pdf\"", colors.green);
  console.log();
  log("  # 群发邮件", colors.gray);
  log("  /dhf-163mail-task --to \"a@163.com,b@163.com\" --subject \"会议\" --body \"周五开会\"", colors.green);
  console.log();
...
function callTaskAPI(input: any): Promise<{ success: boolean; run_id?: string; local_task_id?: string; error?: string }> {
  return new Promise((resolve) => {
    ...
    // Construct the request manually to avoid double encoding
    const postData = `{
      "jsonrpc": "2.0",
      "id": ${Date.now()},
      "method": "tools/call",
...
async function sendMail(options: MailOptions, verbose = false): Promise<void> {
  ...
  // Call task
  log("🚀 启动 163 邮件发送任务...", colors.yellow);
  log(`   任务 ID: ${TASK_ID}`, colors.cyan);
  log(`   发送数据: ${JSON.stringify(taskInput, null, 2)}`, colors.gray);
  console.log();
